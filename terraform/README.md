...
| `apis.tf` | `google_project_service.apis` (for_each, 5 APIs) | Enables every API this module needs, so a brand-new project works with zero manual `gcloud services enable` steps |
...
Resource count: **11 resource blocks** (15 actual resource instances,
counting the 5 API instances under `google_project_service.apis`'s
