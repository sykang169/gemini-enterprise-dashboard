...
### A-9. 🔎 질문 원문 검색 (누가 무엇을 물었나)
사용자가 실제로 입력한 **질문 원문**을 행 단위로 조회·검색. 소스는 `gemini_enterprise_user_activity` 로그(Search=`$.request.query`, StreamAssist=`$.request.query.parts[].text`), 사용자ID는 `userIamPrincipal`에서 직접(조인 불필요).
- **차트**: 표(테이블)
- **데이터 소스**: `v_user_questions` (질문 1건 = 1행)
- **측정기준**: `question_text`, `user_id`, `method`(Search/StreamAssist), `day` (원하면 `timestamp`)
- **측정항목**: 없음(레코드 목록용) — 필요 시 레코드 수만
