...
### A-5. 🤖 개별 에이전트별 사용량 (실제 호출)
- **차트(Chart)**: 누적 막대(Stacked bar) 또는 가로 막대(Bar chart)
- **데이터 소스(Data source)**: `v_agent_usage_daily`
- **기간 측정기준(Date range dimension) → X축(X-axis)**: `day`
- **분류(Breakdown dimension)**: `agent_id`
- **측정항목(Metric)**: `calls`(SUM) — 필요하면 `users`(MAX)로 사용자 수도
- 📖 **수치 읽기**: `calls`=그 에이전트가 **실제로 호출된 횟수**(StreamAssist 요청의 `agentsSpec`에 지정된 것). `agent_id`는 숫자 커스텀 ID이거나 빌트인 슬러그(`deep_research`, `default_idea_generation`)입니다. **표시 이름은 로그에 없습니다** — 필요하면 별도 에이전트 인벤토리와 조인하세요.
- ⚠️ **여기 없는 것**: 에이전트를 지정하지 않은 **일반 어시스턴트 대화는 집계되지 않습니다**(`agentsSpec`이 없음). 이 차트는 "어떤 에이전트가 쓰이나"에 답하지 "총 트래픽"에 답하지 않습니다. 총 트래픽은 A-1(`v_daily_queries`)을 보세요.
- ℹ️ 이전 버전은 `gen_ai`의 `resource.labels.agent_id`를 썼는데, 그 필드는 사용자가 고른 에이전트가 아니라 처리한 어시스턴트라 **항상 `core_assistant` 하나**로 붕괴했습니다. 지금은 그 값이 나오지 않는 게 정상입니다.

### A-5b. 👀 에이전트 페이지 조회 (관심 vs 실사용)
- **차트(Chart)**: 가로 막대(Bar chart) — A-5 옆에 나란히 두면 대비가 드러남
- **데이터 소스(Data source)**: `v_agent_page_views`
- **측정기준(Dimension)**: `agent_id`
- **측정항목(Metric)**: `views`(SUM)
- **필터**: `page_type` = `agent` (에이전트 상세 페이지만; `agent_gallery`·`home` 등은 목록/홈 조회)
- 📖 **수치 읽기**: `views`=사용자가 그 에이전트 **페이지를 연 횟수**(UI 텔레메트리). **호출이 아니라 발견/관심** 신호입니다. **A-5와 나란히 보는 게 핵심** — 조회는 많은데 `calls`가 0에 가까우면 "눌러보긴 하는데 안 쓰는" 에이전트 = 설명·프롬프트·품질 점검 대상.
- ℹ️ 이미 **삭제된 에이전트**도 과거 조회 기록으로 남습니다(API 목록엔 없음).
...
- 🏢 **앱(Gemini Enterprise 앱)별 필터**: `v_daily_queries`·`v_user_questions`·`v_agent_usage_daily`·`v_agent_page_views`에 `engine_id` 차원이 있습니다. `컨트롤 추가 → 드롭다운 목록` → 컨트롤 필드 `engine_id`로 두면 **앱 하나만 골라 대시보드 전체를 좁힐 수 있습니다**(여러 앱이 한 프로젝트에 있을 때 필수). 값은 엔진 ID(`kb-rag-onvoice_1783347133996` 같은 형태)이며 표시 이름은 로그에 없습니다.
...
├───────────────────────────────┼───────────────────────────┤
│ A-4 인용출처 Top(가로막대)       │ A-5 에이전트별 호출          │
│  v_grounding_top_sources       │  v_agent_usage_daily        │
├───────────────────────────────┼───────────────────────────┤
│ A-5b 에이전트 페이지 조회        │ (A-5와 나란히 — 조회는 많은데 │
│  v_agent_page_views            │  호출이 0이면 방치 신호)      │
├───────────────────────────────┴───────────────────────────┤
